...
Original file is located at
    https://colab.research.google.com/drive/1XHQKYJCteDxK1REUkc0Z4x0nSmG4gf4J
"""
...
if uploaded_file:
    ...
    # Calculate total quantity per size from the input dataframe
    total_qty_per_size = {}
    for size in sizes:
        total_qty_per_size[size] = df[size].sum()

    # Add percentage columns in output_df
    output_df['Assorted_Box_Pcs_%'] = output_df.apply(
        lambda row: (row['Assorted_Box_Pcs'] / total_qty_per_size.get(row['SIZE'], 1)) * 100, axis=1
    )

    output_df['Mix_Box_Pcs_%'] = output_df.apply(
        lambda row: (row['Mix_Box_Pcs'] / total_qty_per_size.get(row['SIZE'], 1)) * 100, axis=1
    )

    output_df['Adjusted_Qty_%'] = output_df.apply(
        lambda row: (row.get('Adjusted_Qty', 0) / total_qty_per_size.get(row['SIZE'], 1)) * 100, axis=1
    )

    # Round the percentages nicely
    output_df['Assorted_Box_Pcs_%'] = output_df['Assorted_Box_Pcs_%'].round(2)
    output_df['Mix_Box_Pcs_%'] = output_df['Mix_Box_Pcs_%'].round(2)
    output_df['Adjusted_Qty_%'] = output_df['Adjusted_Qty_%'].round(2)
